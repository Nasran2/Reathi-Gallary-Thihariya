...
$nav = [
    ...
    ['Products', [
        ...
        ['Units', 'units.index'],
        ['Multiple Unit', 'unit-presets.index'],
        ['Brands', 'brands.index'],
    ], 'tabler-box'],
    ['Purchase', [
...
<main class="flex min-h-screen min-w-0 flex-1 flex-col"><header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/70 bg-ivory/90 px-4 backdrop-blur lg:px-8"><button class="rounded-lg p-2 lg:hidden" @click="openMenu()">☰</button><div><div class="text-xs text-slate-400">{{ now()->format('l, d F Y') }}</div><div class="text-sm font-semibold">Main Store</div></div><div class="flex gap-2"><a href="{{ route('pos.remnant') }}" class="btn bg-amber-100 text-amber-800">Remnant POS</a><a href="{{ route('pos.main') }}" class="btn-teal">New Sale</a></div></header><div class="flex-1 p-4 lg:p-8"><div style="position: fixed; top: 1.25rem; right: 1.25rem; z-index: 9999; pointer-events: none; display: flex; flex-direction: column; gap: 0.75rem; width: 20rem; max-width: 100%;">@if(session('success'))<div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 7000)" x-transition.opacity.duration.500ms class="flex items-start gap-3 p-4 rounded-xl shadow-lg border bg-emerald-50 border-emerald-200 text-emerald-800" style="pointer-events: auto;"><div class="flex-1 text-sm">{{ session('success') }}</div><button @click="show = false" class="text-emerald-500 hover:text-emerald-700"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>@endif @if(session('error'))<div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 7000)" x-transition.opacity.duration.500ms class="flex items-start gap-3 p-4 rounded-xl shadow-lg border bg-red-50 border-red-200 text-red-800" style="pointer-events: auto;"><div class="flex-1 text-sm">{{ session('error') }}</div><button @click="show = false" class="text-red-500 hover:text-red-700 mt-0.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>@endif @if($errors->any())<div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 7000)" x-transition.opacity.duration.500ms class="flex items-start gap-3 p-4 rounded-xl shadow-lg border bg-red-50 border-red-200 text-red-800" style="pointer-events: auto;"><div class="flex-1 text-sm"><strong>Please fix the following:</strong><ul class="mt-1 list-disc pl-5">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div><button @click="show = false" class="text-red-500 hover:text-red-700 mt-0.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>@endif</div>@yield('content')</div><footer class="border-t border-slate-200/70 px-4 py-3 text-center text-xs text-slate-400">Software powered by <a class="font-semibold text-teal hover:underline" href="https://twinsofte.com" target="_blank" rel="noopener">Twinsofte.com</a></footer></main></div>@stack('scripts')</body></html>
