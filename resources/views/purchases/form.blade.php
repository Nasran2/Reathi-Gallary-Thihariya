...
@push('scripts')
<script>
function purchaseForm(products, methods, chequesList, editPurchase = null) {
    ...
    return {
        ...
        get landedValue() { return this.systemTotal + (Number(this.extra) || 0) },
        ...
        add() { this.rows.push({ product_id: '', unit_id: '', quantity: 1, cost: 0, system_cost: 0, discount: 0 }) },
        addPayment() { this.payments.push({ method_id: '', amount: 0, reference: '', cheque_number: '', bank: '', cheque_date: '', cheque_id: '' }) },
        methodChanged(pay, e) {
        ...
        productChanged(row) {
            ...
            if (p) {
                row.system_cost = p.average_cost || 0;
                if (this.supplier) { let sup = p.suppliers.find(s => s.id == this.supplier); if (sup) row.cost = sup.last_cost; }
            }
        },
        ...
        systemTotalLine(row) { return (Number(row.quantity) || 0) * (Number(row.system_cost) || 0) },
        get supplierTotal() { return this.rows.reduce((s, r) => s + this.total(r), 0) },
        get systemTotal() { return this.rows.reduce((s, r) => s + this.systemTotalLine(r), 0) },
