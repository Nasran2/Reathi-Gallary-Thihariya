...
@php
if($type==='main'){$catalog=$items->map(fn($p)=>['key'=>'p'.$p->id,'product_id'=>$p->id,'name'=>$p->name,'sku'=>$p->sku,'barcode'=>$p->barcode,'category_id'=>$p->category_id,'colour'=>$p->colour,'stock'=>(float)($p->balances->first()?->quantity??0),'is_favorite'=>(bool)$p->is_favorite,'units'=>$p->productUnits->where('can_sell',true)->map(fn($u)=>['unit_id'=>$u->unit_id,'name'=>$u->unit->name,'symbol'=>$u->unit->symbol,'rate'=>(float)$u->conversion_rate,'price'=>(float)($p->main_selling_price * $u->conversion_rate)])->values()])->values();}
else{$catalog=$items->map(fn($r)=>['key'=>'r'.$r->id,'product_id'=>$r->product_id,'remnant_id'=>$r->id,'name'=>$r->product->name,'sku'=>$r->product->sku,'barcode'=>$r->barcode,'category_id'=>$r->product->category_id,'colour'=>$r->product->colour,'remnant_no'=>$r->remnant_no,'stock'=>(float)$r->remaining_base_quantity,'cost'=>(float)$r->cost_per_base_unit,'normal_price'=>(float)$r->normal_price,'is_favorite'=>(bool)$r->product->is_favorite,'units'=>[['unit_id'=>$r->unit_id,'name'=>$r->unit->name,'symbol'=>$r->unit->symbol,'rate'=>(float)$r->conversion_rate,'price'=>(float)$r->remnant_price]],'display_qty'=>(float)$r->remaining_quantity])->values();}
@endphp
