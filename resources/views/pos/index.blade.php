...
@push('styles')
<style>
/* Deployment-safe POS layout: works even when Vite cannot be rebuilt on the server. */
@media (min-width: 1024px) and (max-width: 1279px) {
    .pos-workspace { grid-template-columns: minmax(0, 1fr) 450px !important; }
}
@media (min-width: 1280px) and (max-width: 1535px) {
    .pos-workspace { grid-template-columns: minmax(0, 1fr) 550px !important; }
    .pos-product-grid { grid-template-columns: repeat(4, minmax(0, 1fr)) !important; }
}
@media (min-width: 1536px) {
    .pos-workspace { grid-template-columns: minmax(0, 1fr) 650px !important; }
    .pos-product-grid { grid-template-columns: repeat(4, minmax(0, 1fr)) !important; }
}
</style>
@endpush
...
<div x-data='pos(@json($catalog),"{{ $type }}", @json($methods))' class="pos-shell -m-3 sm:-m-4 lg:-m-6" :class="cartOpen && 'cart-open'" @keydown.window="handleKey($event)">
 ...
 <div class="max-h-[42vh] min-h-48 overflow-y-auto p-4">
    <template x-for="(line,i) in cart" :key="line.lineKey">
        <div class="mb-2 flex items-center justify-between rounded-xl border border-slate-100 p-1.5 sm:p-2 hover:bg-slate-50 transition">
            ...
            <div class="flex-1 cursor-pointer pr-1 sm:pr-2 min-w-0 flex items-start gap-2" @click="openEditModal(i)">
                <div class="text-sm font-bold text-slate-400 mt-0.5" x-text="(i + 1) + '.'"></div>
                <div class="min-w-0 flex-1">
                    <div class="text-base font-bold text-ink truncate" x-text="line.name" :title="line.name"></div>
                    <div class="flex flex-wrap items-center gap-1 sm:gap-2 mt-1 text-[10px] sm:text-[11px] text-slate-500">
 ...
 <div class="border-t border-slate-100 bg-white p-5"><div class="space-y-3 text-sm font-semibold"><div class="flex justify-between text-slate-700"><span>Items</span><span x-text="cart.length"></span></div><div class="flex justify-between text-slate-700"><span>Subtotal</span><span>Rs <span x-text="money(subtotal)"></span></span></div><div class="flex justify-between text-slate-700"><span>Taxable Amount</span><span>Rs 0.00</span></div><div class="flex justify-between border-t border-slate-200 pt-3 text-lg font-bold text-ink"><span>Payable</span><span>Rs <span x-text="money(total)"></span></span></div></div><div class="mt-4 flex items-center gap-2"><label class="flex items-center gap-1 text-sm"><input type="radio" x-model="globalDiscountType" value="fixed"> Fixed</label><label class="flex items-center gap-1 text-sm"><input type="radio" x-model="globalDiscountType" value="percentage"> %</label><input class="w-full !px-3" type="number" min="0" step="0.01" x-model.number="globalDiscountValue"></div><button type="button" @click="duePay()" class="mt-4 w-full rounded-xl border border-amber-200 bg-amber-50 py-3 font-semibold text-amber-700 transition hover:bg-amber-100" :disabled="submitting"><span x-text="submitting?'Processing...':'Due & Print (No Payment)'"></span></button><div class="mt-4 grid grid-cols-4 gap-2"><button type="button" @click="clearCart()" class="rounded-xl border border-red-100 bg-red-50 py-2.5 text-xs font-bold text-red-600 transition hover:bg-red-100">Clear</button><button type="button" @click="holdCart()" class="rounded-xl border border-amber-100 bg-amber-50 py-2.5 text-xs font-bold text-amber-600 transition hover:bg-amber-100">Hold (F6)</button><button type="button" @click="quickPay()" class="rounded-xl bg-emerald-400 py-2.5 text-xs font-bold text-white transition hover:bg-emerald-500">Quick (F10)</button><button type="button" @click="openPaymentModal()" class="rounded-xl bg-indigo-400 py-2.5 text-xs font-bold text-white transition hover:bg-indigo-500">Pay (F4)</button></div></div>
 </aside></div>
 ...
 <form id="checkout-form" method="post" action="{{ route('pos.checkout') }}" style="display:none;">@csrf<input type="hidden" name="sale_type" value="{{ $type }}"><input type="hidden" name="store_id" value="{{ $store->id }}"><input type="hidden" name="idempotency_key" :value="token"><input type="hidden" name="customer_id" :value="customer"><input type="hidden" name="notes" :value="sellNote"><input type="hidden" name="staff_note" :value="staffNote"><input type="hidden" name="edit_sale_id" :value="editSaleId"></form>
     ...
     <div class="w-full max-w-xl rounded-2xl bg-white shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" @click.outside="manageFavoritesModal=false">
         ...
         <div class="p-6 overflow-y-auto bg-slate-50 flex-1">
             <div class="mb-6 relative">
                 ...
                 <div x-show="favSearch && favSearchResults.length > 0" class="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg overflow-hidden max-h-60 overflow-y-auto">
             ...
             <div class="space-y-2">
                <template x-for="(fav, idx) in favoriteList" :key="fav.product_id">
                    <div class="flex items-center justify-between bg-white border border-slate-200 rounded-lg p-3 shadow-sm transition-all"
                         :draggable="favSearch === ''"
                         @dragstart="dragStart($event, idx)"
                         @dragover="dragOver($event)"
                         @drop="dragDrop($event, idx)"
                         @dragend="dragEnd($event)"
                         :class="{'opacity-50 border-teal scale-[0.98]': draggedIdx === idx}">
                        
                        <div class="flex items-center gap-3 flex-1 min-w-0 pr-4">
                            <div class="cursor-grab active:cursor-grabbing text-slate-300 hover:text-slate-500 p-1" title="Drag to reorder">
                                <i class="ti ti-grip-vertical"></i>
                            </div>
                            
                            <div class="flex flex-col items-center justify-center shrink-0" title="Edit order number">
                                <input type="number" class="w-12 h-8 px-1 text-center border border-slate-200 rounded text-sm font-semibold text-slate-700 bg-slate-50 focus:bg-white focus:border-teal outline-none transition-colors" 
                                       min="1" :max="favoriteList.length" 
                                       :value="idx + 1" 
                                       @change="moveFav(idx, parseInt($event.target.value) - 1)">
                            </div>
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            <button type="button" class="p-1 text-slate-400 hover:text-teal hover:bg-teal/10 rounded disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-slate-400" :disabled="idx === 0" @click="moveFavUp(idx)" title="Move Up"><i class="ti ti-arrow-up"></i></button>
                            <button type="button" class="p-1 text-slate-400 hover:text-teal hover:bg-teal/10 rounded disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-slate-400" :disabled="idx === favoriteList.length - 1" @click="moveFavDown(idx)" title="Move Down"><i class="ti ti-arrow-down"></i></button>
                            <div class="w-px h-5 bg-slate-200 mx-1"></div>
                            <button type="button" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors" @click="removeFromFavorites(idx)" title="Remove from favorites"><i class="ti ti-trash"></i></button>
                        </div>
                    </div>
                </template>
 ...
 <div x-show="showHeldBillsModal" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
     <div class="w-full max-w-2xl rounded-2xl bg-white shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" @click.outside="showHeldBillsModal=false">
         <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
             <h2 class="font-serif text-xl font-semibold text-ink">Held Bills</h2>
             <button type="button" @click="showHeldBillsModal=false" class="text-slate-400 hover:text-slate-600"><i class="ti ti-x text-xl"></i></button>
         </div>
         <div class="p-6 overflow-y-auto bg-slate-50 flex-1">
             <div class="space-y-3">
                <template x-for="(bill, idx) in heldBills" :key="bill.id">
                    <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
                        <div>
                            <div class="font-bold text-slate-800 text-lg">Rs. <span x-text="money(bill.total)"></span></div>
                            <div class="text-sm text-slate-500 mt-1 flex gap-3">
                                <span><i class="ti ti-clock"></i> <span x-text="bill.date"></span></span>
                                <span><i class="ti ti-box"></i> <span x-text="bill.itemsCount"></span> items</span>
                            </div>
                        </div>
                        <div class="flex gap-2 shrink-0">
                            <button type="button" class="btn-teal text-xs py-1.5" @click="restoreHeldBill(idx)"><i class="ti ti-arrow-back-up mr-1"></i> Restore</button>
                            <button type="button" class="btn-soft text-xs py-1.5 text-red-500 border-red-200 bg-red-50 hover:bg-red-100" @click="deleteHeldBill(idx)"><i class="ti ti-trash"></i></button>
                        </div>
                    </div>
                </template>
                <div x-show="heldBills.length === 0" class="text-center py-12 text-slate-400">
                    <i class="ti ti-receipt text-4xl mb-2"></i>
...
@push('scripts')<script>function pos(catalog,type,methods){return{catalog,type,methods,search:'',category:'',cart:@json(session('edit_cart', [])),customer:'{{ session('edit_customer', '') }}',editSaleId:'{{ session('edit_sale_id', '') }}',editInvoiceNo:'{{ session('edit_invoice_no', '') }}',token:'{{ \Illuminate\Support\Str::uuid() }}',submitting:false,showPaymentModal:false,paymentRows:[],sellNote:'',staffNote:'',paymentError:'',editModal:false,editingLineIndex:null,
...
showHeldBillsModal:false,heldBills:JSON.parse(localStorage.getItem('pos_held_bills_'+type)||'[]'),
...
saveHeldBills(){
    localStorage.setItem('pos_held_bills_'+this.type,JSON.stringify(this.heldBills));
},
restoreHeldBill(index){
    let bill=this.heldBills[index];
    this.cart=JSON.parse(JSON.stringify(bill.cart));
    this.customer=bill.customer;
    this.sellNote=bill.sellNote;
    this.staffNote=bill.staffNote;
    this.deleteHeldBill(index);
    this.showHeldBillsModal=false;
},
deleteHeldBill(index){
    this.heldBills.splice(index,1);
    this.saveHeldBills();
},
get filtered(){let s=this.search.toLowerCase();return this.catalog.filter(x=>(!this.category||String(x.category_id)===this.category)&&(!s||`${x.name} ${x.sku} ${x.barcode||''} ${x.remnant_no||''} ${x.colour||''}`.toLowerCase().includes(s))).sort((a,b)=>{if(b.is_favorite!==a.is_favorite)return (b.is_favorite?1:0)-(a.is_favorite?1:0);return (a.favorite_order||0)-(b.favorite_order||0)})},addUniqueSearch(){let matches=this.filtered;if(matches.length!==1)return;this.add(matches[0]);this.search='';},autoAddUnique(){if(!this.search.trim())return;this.addUniqueSearch();},available(item,u){return u?Number(item.stock/u.rate).toFixed(3):0},unitPrice(item){return item.units[0]?item.units[0].price:0},unitSymbol(item){return item.units[0]?item.units[0].symbol:''},availabilityLabel(item){let unit=item.units[0];if(!unit)return '';return this.type==='remnant'?item.display_qty+' '+unit.symbol:this.available(item,unit)+' '+unit.symbol},itemFieldName(index,field){return `items[${index}][${field}]`},methodIsCheque(id){let method=this.methods.find(item=>item.id==id);return Boolean(method&&method.code.includes('cheque'))},addPaymentRow(){this.paymentRows.push({amount:0,method_id:'',note:'',cheque_number:'',bank:'',cheque_date:''})},removePaymentRow(index){this.paymentRows.splice(index,1)},removeCartLine(index){this.cart.splice(index,1)},add(item){let u=item.units[0];if(!u)return;let key=item.key+'-'+u.unit_id+'-'+Date.now()+'-'+Math.random().toString(36).substr(2,5);this.cart.push({...item,lineKey:key,unit_id:u.unit_id,quantity:1,price:u.price,discount_type:'fixed',discount_value:0,maxDisplay:Number(item.stock/u.rate).toFixed(3)})},unitChanged(line){let u=line.units.find(x=>x.unit_id==line.unit_id);if(!u)return;line.price=u.price;line.maxDisplay=Number(line.stock/u.rate).toFixed(3);line.lineKey=line.key+'-'+u.unit_id+'-'+Date.now()},lineDiscountAmount(l){let q=Number(l.quantity)||0;let p=Number(l.price)||0;let v=Number(l.discount_value)||0;if(l.discount_type==='percentage'){return (q*p)*(v/100);}return v;},lineTotal(l){return Math.max(0,(Number(l.quantity)||0)*(Number(l.price)||0)-this.lineDiscountAmount(l))},get subtotal(){return this.cart.reduce((s,l)=>s+(Number(l.quantity)||0)*(Number(l.price)||0),0)},get discount(){let ld=this.cart.reduce((s,l)=>s+this.lineDiscountAmount(l),0);let sub=this.subtotal-ld;let v=Number(this.globalDiscountValue)||0;let g=this.globalDiscountType==='percentage'?(sub*v/100):v;return ld+g;},get total(){return Math.max(0,this.subtotal-this.discount)},get totalPaying(){return this.paymentRows.reduce((s,r)=>s+(Number(r.amount)||0),0)},get changeReturn(){return Math.max(0,this.totalPaying-this.total)},get balance(){return Math.max(0,this.total-this.totalPaying)},money(v){return Number(v||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})},openPaymentModal(){if(this.cart.length===0)return;if(this.paymentRows.length===0){this.paymentRows=[{amount:this.total,method_id:{{ $methods->firstWhere('code','cash')?->id ?? '""' }},note:'',cheque_number:'',bank:'',cheque_date:''}];}this.showPaymentModal=true;},duePay(){if(this.cart.length===0)return;if(!this.customer){this.showError("Please select the customer to complete this");return;}this.paymentRows=[];this.submitSale();},quickPay(){if(this.cart.length===0)return;this.paymentRows=[{amount:this.total,method_id:{{ $methods->firstWhere('code','cash')?->id ?? '""' }},note:'',cheque_number:'',bank:'',cheque_date:''}];this.submitSale();},handleKey(e){if(this.showPaymentModal||this.editModal||this.showHeldBillsModal)return;if(e.key==='F4'){e.preventDefault();this.openPaymentModal();}if(e.key==='F10'){e.preventDefault();this.quickPay();}if(e.key==='F6'){e.preventDefault();this.holdCart();}},openManageFavorites(){this.favoriteList=this.catalog.filter(x=>x.is_favorite).sort((a,b)=>a.favorite_order-b.favorite_order);this.favSearch='';this.manageFavoritesModal=true;},get favSearchResults(){if(!this.favSearch.trim())return[];let s=this.favSearch.toLowerCase();let addedIds=this.favoriteList.map(f=>f.product_id);return this.catalog.filter(x=>!addedIds.includes(x.product_id)&&(!s||`${x.name} ${x.sku}`.toLowerCase().includes(s)));},addToFavorites(item){this.favoriteList.push({...item});this.favSearch='';},removeFromFavorites(idx){this.favoriteList.splice(idx,1);},moveFavUp(idx){if(idx>0){let temp=this.favoriteList[idx];this.favoriteList[idx]=this.favoriteList[idx-1];this.favoriteList[idx-1]=temp;}},moveFavDown(idx){if(idx<this.favoriteList.length-1){let temp=this.favoriteList[idx];this.favoriteList[idx]=this.favoriteList[idx+1];this.favoriteList[idx+1]=temp;}},dragStart(event,idx){this.draggedIdx=idx;event.dataTransfer.effectAllowed='move';},dragOver(event){event.preventDefault();event.dataTransfer.dropEffect='move';},dragDrop(event,idx){event.preventDefault();if(this.draggedIdx!==null){this.moveFav(this.draggedIdx,idx);this.draggedIdx=null;}},dragEnd(event){this.draggedIdx=null;},moveFav(from,to){if(from===to||to<0||to>=this.favoriteList.length){this.favoriteList=[...this.favoriteList];return;}let item=this.favoriteList.splice(from,1)[0];this.favoriteList.splice(to,0,item);},saveFavoritesOrder(){this.savingFavorites=true;let ids=this.favoriteList.map(x=>x.product_id);fetch('{{ route("products.favorites.reorder") }}',{method:'POST',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content'),'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({favorite_ids:ids})}).then(res=>res.json()).then(data=>{this.savingFavorites=false;if(data.success){this.catalog.forEach(item=>{let favIdx=ids.indexOf(item.product_id);if(favIdx!==-1){item.is_favorite=true;item.favorite_order=favIdx+1;}else{item.is_favorite=false;item.favorite_order=0;}});this.manageFavoritesModal=false;}}).catch(e=>{this.savingFavorites=false;alert('Error saving favorites');});},submitSale(){if(this.cart.length===0)return;if(this.paymentRows.length && this.paymentRows.some(r=>!r.method_id)){this.showError("Please select a payment method for all rows.");return;}let missingCheque=this.paymentRows.some(r=>{let m=this.methods.find(m=>m.id==r.method_id);return m?.code.includes('cheque')&&(!r.cheque_number||!r.bank||!r.cheque_date);});if(missingCheque){this.showError("Cheque payments require cheque number, bank, and date.");return;}let missingRef=this.paymentRows.some(r=>{let m=this.methods.find(m=>m.id==r.method_id);return m?.requires_reference&&!r.note&&!m?.code.includes('cheque');});if(missingRef){this.showError("A payment reference (note) is required for the selected method.");return;}let hasCheque=this.paymentRows.some(r=>this.methods.find(m=>m.id==r.method_id)?.code.includes('cheque'));let hasDue=this.totalPaying<this.total;if((hasCheque||hasDue)&&!this.customer){this.showError("Please select the customer to complete this");return;}this.submitting=true;let form=document.getElementById('checkout-form');
let formData = new FormData(form);
let ld=this.cart.reduce((s,l)=>s+this.lineDiscountAmount(l),0);let sub=this.subtotal-ld;let v=Number(this.globalDiscountValue)||0;let g=this.globalDiscountType==='percentage'?(sub*v/100):v;
this.cart.forEach((c,i)=>{
    let ls=(Number(c.quantity)||0)*(Number(c.price)||0)-this.lineDiscountAmount(c);
    let pg=sub>0?(ls/sub)*g:0;
    let td=this.lineDiscountAmount(c)+pg;
    formData.append(`items[${i}][product_id]`, c.product_id);
    if(c.remnant_id) formData.append(`items[${i}][remnant_id]`, c.remnant_id);
    formData.append(`items[${i}][unit_id]`, c.unit_id);
    formData.append(`items[${i}][quantity]`, c.quantity||0);
    formData.append(`items[${i}][unit_price]`, c.price||0);
    formData.append(`items[${i}][discount_amount]`, td.toFixed(4));
});
this.paymentRows.forEach((r,i)=>{
    formData.append(`payments[${i}][payment_method_id]`, r.method_id);
    formData.append(`payments[${i}][amount]`, r.amount||0);
    if(r.note) formData.append(`payments[${i}][reference]`, r.note);
    if(this.methods.find(m=>m.id==r.method_id)?.code.includes('cheque')){
        formData.append(`payments[${i}][cheque_number]`, r.cheque_number||'');
        formData.append(`payments[${i}][bank]`, r.bank||'');
        formData.append(`payments[${i}][cheque_date]`, r.cheque_date||'');
    }
});
...
.then(res => {
    if (res.ok) {
        ...
        let msg = res.data.message || 'An error occurred';
