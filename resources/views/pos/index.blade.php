...
<div x-data='pos(@json($catalog),"{{ $type }}", @json($methods))' class="-m-4 lg:-m-8" @keydown.window="handleKey($event)">
 ...
 <div class="max-h-[42vh] min-h-48 overflow-y-auto p-4"><template x-for="(line,i) in cart" :key="line.lineKey"><div class="mb-3 rounded-xl border border-slate-100 p-3"><input type="hidden" :name="itemFieldName(i, 'product_id')" :value="line.product_id"><input type="hidden" :name="itemFieldName(i, 'remnant_id')" :value="line.remnant_id||''"><div class="flex justify-between gap-2"><div><div class="text-sm font-semibold" x-text="line.name"></div><div class="text-[11px] text-slate-400" x-text="line.remnant_no||line.sku"></div></div><button type="button" class="text-red-400" @click="removeCartLine(i)">×</button></div><div class="mt-3 grid grid-cols-3 gap-2"><div><label>Unit</label><select class="w-full !px-2" :name="itemFieldName(i, 'unit_id')" x-model.number="line.unit_id" @change="unitChanged(line)"><template x-for="u in line.units"><option :value="u.unit_id" x-text="u.symbol"></option></template></select></div><div><label>Quantity</label><input class="w-full !px-2" type="number" min="0.000001" step="0.000001" :max="line.maxDisplay" :name="itemFieldName(i, 'quantity')" x-model.number="line.quantity" required></div><div><label>Auto unit price</label><input class="w-full !px-2 bg-slate-50" type="number" min="0" step="0.0001" :name="itemFieldName(i, 'unit_price')" x-model.number="line.price" readonly required></div></div><div class="mt-2 grid grid-cols-2 gap-2"><div><label>Discount</label><input class="w-full !px-2" type="number" min="0" step="0.01" :name="itemFieldName(i, 'discount_amount')" x-model.number="line.discount"></div><div class="flex items-end justify-end pb-2 text-sm font-bold">Rs. <span x-text="money(lineTotal(line))"></span></div></div></div></template><div x-show="cart.length===0" class="py-14 text-center"><div class="text-4xl text-slate-200">◫</div><p class="mt-2 text-sm text-slate-400">Choose an item to begin</p></div></div>
