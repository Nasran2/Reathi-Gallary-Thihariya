<table class="w-full text-left text-sm text-slate-500">
    <thead class="bg-slate-50 text-xs uppercase text-slate-700">
        <tr>
            ...
            <th class="px-6 py-3 text-right text-amber-600">Due Amount</th>
        </tr>
    </thead>
    <tbody>
        @forelse($sales as $sale)
            <tr class="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td class="px-6 py-4 text-right font-bold text-amber-600">Rs. {{ number_format($sale->due_total, 2) }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="6" class="px-6 py-12 text-center">
                    <h3 class="text-lg font-bold text-slate-700 mb-1">No due bills found for this period.</h3>
